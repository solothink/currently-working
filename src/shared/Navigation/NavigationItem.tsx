...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderDropdownMenuNavlink = (item: NavItemType) => {
    return (
      ...
      >
        {item.icon && (
          <span className="w-9 h-9 flex items-center justify-center rounded-full bg-neutral-100 text-xl dark:bg-neutral-800 dark:text-neutral-300 mr-2">
            <i className={item.icon}></i>
          </span>
        )}
        {item.name}

        {item.type && (item.children || item.megaMenu) && (
          <ChevronDownIcon
